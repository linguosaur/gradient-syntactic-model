...
[sentFN,parseFN,phraseNeighFN,corpusFN] = sys.argv[1:5]
	...
	for span,phrase in phrases.iteritems():
		...
		if i < span[0]:
			...
			for k in range(1,wordDist+1):
				dir1 = word2phraseNeigh[(word,'neigh('+phrase+')','<',k)] = findCondProb([word],neighbours[phrase],i,span[0],'<',k,splitCorpus)
				dir2 = word2phraseNeigh[('neigh('+phrase+')',word,'>',k)] = findCondProb(neighbours[phrase],[word],span[0],i,'>',k,splitCorpus)
				threshold = 0.0
...
for k in sorted(word2phraseNeigh.keys(), key=operator.itemgetter(0,1,2,3)):
    sys.stderr.write('word2phraseNeigh' + repr(k) + '\t' + repr(word2phraseNeigh[k]) + '\n')
	...
	for span,phrase in sortedPhrases:
		if i in range(span[0],span[1]): sys.stdout.write('--')
			...
			if i >= span[1]:
				direction = '>'
				dist = i - span[1] + 1
			if (word,'neigh('+phrase+')',direction,dist) in word2phraseNeigh:
				sys.stdout.write(repr(word2phraseNeigh[(word,'neigh('+phrase+')',direction,dist)][7]))
		sys.stdout.write('\t')
	sys.stdout.write('\n')
