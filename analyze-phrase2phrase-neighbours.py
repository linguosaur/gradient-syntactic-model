	...
	for span2,phrase2 in phrases.iteritems():
		...
		if span1[1] <= span2[0]:
			...
			for k in range(1,dist+1):
				dir1 = phrase2phrase[('neigh('+phrase1+')','neigh('+phrase2+')','<',k)] = findCondProb(neighbours[phrase1],neighbours[phrase2],span1[0],span2[0],'<',k,splitCorpus)
				dir2 = phrase2phrase[('neigh('+phrase2+')','neigh('+phrase1+')','>',k)] = findCondProb(neighbours[phrase2],neighbours[phrase1],span2[0],span1[0],'>',k,splitCorpus)
				threshold = 0.0
...
for span1,phrase1 in sortedPhrases:
	sys.stdout.write('neigh('+phrase1+')' + '\t')
	for span2,phrase2 in sortedPhrases:
		if overlap(span1,span2): sys.stdout.write('--')
			...
			if span1[1] <= span2[0]:
				...
				direction = '>'
				dist = span1[0] - span2[1] + 1
			if ('neigh('+phrase1+')','neigh('+phrase2+')',direction,dist) in phrase2phrase:
				sys.stdout.write(repr(phrase2phrase[('neigh('+phrase1+')','neigh('+phrase2+')',direction,dist)][7]))
		sys.stdout.write('\t')
	sys.stdout.write('\n')
